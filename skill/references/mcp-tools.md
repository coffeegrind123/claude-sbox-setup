...
And a third, **real-world source** pipeline — **`codesearch_*`**: queries the public Facepunch REST endpoint `public.facepunch.com/sbox/code/search/1/` (the no-auth backend behind [sbox.game/codesearch](https://sbox.game/codesearch)) for the source of *every open-source package*. Where `schema_*` tells you a method's *signature*, codesearch tells you how people *actually call it* in shipped code: `codesearch_search(query)` returns ranked hits (package, file, snippet, source URL), then `codesearch_get_file(...)` pulls the whole file and `codesearch_list_files(...)` walks a package's tree. **Queries leave the machine** (they hit Facepunch's servers). No driver/install — plain HTTP. See § Code search.

Two more **live sbox.game** surfaces: **`release_notes`** reads the [changelog](https://sbox.game/release-notes) via the same REST host (`/news/platform` — weekly update posts with titled sections); and **`forum_*`** browses + searches the [community forum](https://sbox.game/f) (live discussion, bug reports, dev chat, announcements — `forum_list_categories` → `forum_browse_category` → `forum_read_thread`, plus `forum_search`). The forum is the **only** surface still using a headless-Chromium driver (it's a Blazor Server app with no REST endpoint; one-time `codesearch_install_driver`). See § Community forum and § Release notes.
...
**How it works (plain REST — no driver, no install)**: the codesearch tools call the public Facepunch backend directly: `GET https://public.facepunch.com/sbox/code/search/1/?q=&type=&year=&ident=` (the same no-auth host the engine's own `Backend` uses; the `sbox.game/codesearch` Blazor page is just a thin client over it). It returns JSON `{TotalCount, Files:[…]}` where each file's `Code` is the **complete source**. No headless browser, no Chromium, nothing to install. **Queries still leave the machine** (the query string hits Facepunch's servers) — for identifiers from private project source, prefer `schema_*` + `docs_*`.

Two backend quirks the tools surface for you: results are a **capped top-N (~30) with no paging** — `total_results` / `total_in_package` + a `truncated` flag tell you when a list was cut off; and `ident=<org.package>` is a real server-side package filter (an empty query + an ident enumerates that package's tree, which is how `list_files` works).

| Tool | What it does |
|---|---|
| `codesearch_search(query, type?, year?, ident?, limit?)` | Search package source. Returns `{total_results, returned, truncated, hits:[{package, file, kind, code_kind, source_url, snippet}]}`. `query` is free-text or a symbol (`"Physics.Trace"` quoted for phrase). `type` filters package kind (`library`/`game`/`code`/`editor`/`unittest`); `year` filters publish year; `ident` scopes to one package (e.g. `facepunch.sandbox`). `snippet` is the head of the file; `source_url` feeds straight into `codesearch_get_file`. |
| `codesearch_get_file(source_url \| org+package+file)` | Fetch the COMPLETE source of one file. Pass a hit's `source_url`, or `org`+`package`+`file` explicitly (e.g. `facepunch` / `sandbox` / `Player/NoclipMoveMode.cs`). Returns full `content` + `line_count` + `language`. |
| `codesearch_list_files(org, package)` | Enumerate a package's source tree (flat list of file paths). Reports `total_in_package` + `truncated` — the backend caps the list ~30; narrow with `codesearch_search` for a file beyond the cap. |
| `codesearch_status` | Transport diagnostic — reports `transport:"rest"` + the endpoint. Read-only; run first when codesearch fails (a failure means the backend or your network is unreachable). |
| `codesearch_restart` | Retained no-op (codesearch is REST-backed + stateless; nothing to restart). |
| `codesearch_install_driver(force?, timeout_seconds?)` | **Not needed for codesearch anymore** — this builds the headless-Chromium driver that only the `forum_*` tools use. See § Community forum. |
...
## Community forum (`forum_*`)

Browse + search the **s&box community forum** at [sbox.game/f](https://sbox.game/f): live discussion — bug reports, dev chat, announcements, Q&A, per-package forums. Distinct from the docs families: `docs_*`/`learn_*` are authored prose + tutorials; `forum_*` is what people are *actually saying* right now. Reach for it for "has anyone hit this crash?", "what's the community saying about X?", "find the thread where garry explained Y", or to read a News Threads announcement's replies.

**Needs the headless-Chromium driver** (the *only* tool family that still does — codesearch + release_notes moved to plain REST). The forum is a **Blazor Server** app: threads stream over a SignalR circuit and are absent from the raw HTML, so there's no REST endpoint to hit — `forum_*` drives a headless Chromium (Microsoft.Playwright) that loads the page, lets the circuit render, and scrapes the DOM. The driver is a **prebuilt DLL loaded at runtime** (`Assembly.LoadFrom`) from the game's global store `<game>/.claude-sbox/codesearch-driver/runtime/`; its source + build scripts live in `claude-sbox-setup`, never in the published addon. If a forum call returns `forum_driver_unavailable`, run **`codesearch_install_driver`** once (~1–3 min for restore + Chromium download — the install tool keeps its `codesearch_` name for back-compat). If it returns `forum_driver_outdated`, the deployed driver predates these ops — run `codesearch_install_driver force:true` then **restart the editor** (the driver DLL is `Assembly.LoadFrom`-cached). **Queries leave the machine.**

| Tool | What it does |
|---|---|
| `forum_list_categories` | The forum index: every category with `slug`, `name`, `icon`, `description`, thread/post counts, and latest thread. **Start here** — the `slug` feeds `forum_browse_category` (e.g. `general`, `devhelp`, `api`, `newsposts`, `crash`, `editor`, `mapping`, plus per-package forums). |
| `forum_browse_category(category)` | The ~50 most-recent threads in a category (pass the slug). Each: `thread_id`, `title`, `url`, `author`, `created`, `replies`, `views`, last-post info. **No anonymous pagination** — only the latest ~50 show; use `forum_search` for older/specific threads. |
| `forum_read_thread(url \| category+thread_id, page?)` | One page of a thread (30 posts/page). Each post: `index` (#N), `author` (+url), `author_score` (trophy), join date, post count, `time`/`time_abs`, full `content`, `ratings`. Pass a hit's `url` (incl. `/f/post/<id>` permalinks from search) or `category`+`thread_id`; `page` for later posts. |
| `forum_search(query)` | Full-text search via the site's own index. Returns the top ~50 hits — `title`, `category`, matched `snippet`, permalink `url`, `footer` ("time ago by author") — plus the true `total_results`. The way to find older/specific threads; refine the query to narrow. |

## Release notes (`release_notes`)

Read the **s&box changelog** via the platform-news REST endpoint `GET https://public.facepunch.com/sbox/news/platform` (the same feed behind `sbox.game/release-notes` — no auth, no driver). Returns the weekly "Update YY.MM.DD" posts newest-first, each with `version`, `title`, `date`, `summary`, and the changes as titled `sections` (each `{title, content}`, with the post's HTML reduced to plain text). Use for "what changed recently?", "when did API X land or change?", "is my crash a known issue?", or to confirm whether a behaviour shifted in a recent update.

| Tool | What it does |
|---|---|
| `release_notes(limit?, version?)` | `limit` = how many update posts back (default 10, newest first). `version` = substring filter matched against each post's version + title (e.g. `'26.05'` or an exact `'26.05.27'`). Returns `{versions:[{version, title, date, url, summary, sections:[{title, content}]}]}`. (Note: `sections` are the post's titled blocks, not pre-bucketed added/fixed/removed groups — that grouping was a website-only view.) |
...
This file documents the tool families with a UX or correctness story worth elaborating. **For everything else**: the ~730 tools the bridge exposes, including subsystems trimmed from this doc to keep it scannable (`host_*`, `prefab_*`, `gameobject_*` lifecycle + tags, `add_physics`/`_collider`/`_joint`, `scene_overlap_*`, `lighting_*`/`envmap_*`/`indirect_light_volume_*`/`fog_*`/`skybox_*`/`decal_*`, `terrain_*`, `surface_list`/`resource_list_by_type`, `create_*` scaffolders, `debug_draw_*`, `scene_trace_*`, `rigidbody_*`, `navmesh_*`, `anim_*` (incl. bone posing + `anim_capture_events`), `sound_*`, `particle_*`, `dispatcher_*`, Roslyn devtools, `find_*` cross-ref, `language_*`, `material_*`, `mapview_*`, `mapentity_*`, `hammer_node_*`, `audio_*`, `shader_*`, `game_*`, `vr_*`, `account_*`, project publishing, and more): see `tool-families.md` for a one-liner discovery index. Grep that file for a topical keyword first; reach for `list_tools` only when neither file mentions the family you need.
