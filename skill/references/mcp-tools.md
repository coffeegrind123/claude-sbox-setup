...
That same headless-Chromium driver also powers two more **live sbox.game** surfaces (same Blazor-scrape constraint, same one install): **`forum_*`** browses + searches the [community forum](https://sbox.game/f) (live discussion, bug reports, dev chat, announcements — `forum_list_categories` → `forum_browse_category` → `forum_read_thread`, plus `forum_search`), and **`release_notes`** reads the [changelog](https://sbox.game/release-notes) (per-version added/improved/fixed/removed/known-issues). See § Community forum and § Release notes.
...
**Shares the codesearch driver.** The forum is the same **Blazor Server** app as codesearch (threads stream over a SignalR circuit, absent from raw HTML), so `forum_*` rides the *same* headless-Chromium driver — one install, one browser. If a forum call returns `forum_driver_unavailable`, run **`codesearch_install_driver`** (same as for codesearch). If it returns `forum_driver_outdated`, the deployed driver predates these ops — run `codesearch_install_driver force:true` then **restart the editor** (the driver DLL is `Assembly.LoadFrom`-cached). **Queries leave the machine.**
...
Read the **s&box changelog** at [sbox.game/release-notes](https://sbox.game/release-notes) — per-version groups, newest first, each with `version`, `date`, and changes grouped into `added` / `improved` / `fixed` / `removed` / `known_issues`. Use for "what changed recently?", "when did API X land or change?", "is my crash a known issue?", or to confirm whether a behaviour shifted in a recent update. **Also rides the shared codesearch driver** (same Blazor-scrape constraint, same install/`*_driver_outdated` story as `forum_*`).

| Tool | What it does |
|---|---|
| `release_notes(limit?, version?)` | `limit` = how many version groups back (default 10, newest first). `version` = substring filter across all versions (e.g. `'26.05'` or an exact `'26.05.27'`). Returns `{versions:[{version, date, sections:{added,improved,fixed,removed,known_issues}}]}`. |
