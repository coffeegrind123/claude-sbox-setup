...
| Common gotchas (namespace surprises, signature traps, set_property coercion — bool/Vector3/float/asset-handles now work, runtime-only props no-op in edit mode, bone GET=world vs SET=model space, Components.Get skips disabled, live runtime debugging, screenshot location, codesearch driver lifecycle + privacy (shared by forum_*/release_notes), auto_* naming, widget_drag rejections) | `references/gotchas.md` |
...
| "What's the community saying about X" / find a forum thread / read a bug report or announcement | Don't read a file: `forum_search(query)` for the site's own index (older/specific threads), or `forum_list_categories` → `forum_browse_category(slug)` for recent threads, then `forum_read_thread(url)` for the posts. Shares the codesearch driver (`forum_driver_outdated` → `codesearch_install_driver force:true` + restart). Queries leave the machine. See `references/mcp-tools.md` § Community forum. |
| "What changed in the last update" / when did an API land or change / is this a known issue | Don't read a file: call `release_notes(limit?)` for recent version groups (added/improved/fixed/removed/known_issues), or `release_notes(version: "26.05")` to filter. Shares the codesearch driver. See `references/mcp-tools.md` § Release notes. |
