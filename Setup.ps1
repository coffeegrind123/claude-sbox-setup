...
.EXAMPLE
    .\Setup.ps1 -DryRun
    Preview what each patch would do without writing.

.EXAMPLE
    .\Setup.ps1 -Force
    Re-apply every patch from scratch, bypassing the "already applied"
    idempotency probe.
#>
[CmdletBinding()]
param(
    [switch]$DryRun,
    [switch]$Force,
    # Accepted as a no-op alias for backwards compatibility with scripts /
    # muscle memory from the old flag-gated design (when -Dev opted in to
    # patch-apply and the default was a print-and-exit info mode). Both
    # audiences now run the same Setup, so the flag is redundant but
    # harmless. Don't document in the synopsis to avoid suggesting it's
    # required.
    [switch]$Dev
)
