...
## Game-code whitelist blocks `System.Reflection`; EDITOR addons run full-trust

Game/runtime assemblies are compiled under the s&box access whitelist: `System.Type.GetType`, `Type.GetMethod`, `MethodBase.Invoke` (and reflection generally) emit `SB1000 "... is not allowed when whitelist is enabled"` and the build fails. **Editor addon assemblies (the project's `Editor/` csproj, toolbase, etc.) are NOT whitelisted** — they can use full reflection, file IO, and reference engine dlls directly. Pattern: when you need a trusted/host-only capability for a game, put it in the editor assembly, not game code.

## `[Event]` handler signatures: parameter COUNT must match the event's args

`EventSystem.BuildDelegate` requires the handler's parameter count to equal the number of args the event is dispatched with — a mismatch throws and you see `EventSystem | Error calling event 'X' on 'Type'` (the handler silently never runs). A **parameterless** method on an event that passes args fails this way. Two fixes: declare the exact typed parameter, or use a single `object` parameter (accepted for any 1-arg event via reflection-invoke). Common editor events and their one arg: `editor.created` → `EditorMainWindow`, `compile.complete` → `CompileGroup`. `[Event]` is `AllowMultiple=true`, so you can stack several on one `static void Handler(object _ = null)`. Note `compile.complete` re-fires on every hotload, but the assembly you just recompiled only registers its listener AFTER that compile's event has fired — so a freshly-added handler first runs on the NEXT compile, not the one that introduced it.

## MCP scene edits can silently revert on reload; `save_scene` "had_unsaved_changes:false" is misleading — verify against disk

`gameobject_create`/`gameobject_destroy`/`set_property` mutate the **in-memory** edit session through editor undo, but they don't reliably flag the session dirty for `save_scene`, which then returns `had_unsaved_changes:false` even though you just changed things. Worse, any scene **reload** in between — pressing Play/Stop, an editor relaunch, or opening another scene — restores the on-disk scene and silently discards your un-persisted edits (tell-tale signs: the scene-root name reverts to its saved casing and objects like `editor_camera` come back with fresh GUIDs). The fix: after a batch of scene mutations, call `save_scene`, then **confirm against the file on disk** (e.g. `grep '"Name"' Assets/scenes/<scene>.scene`) rather than trusting the return value. If a delete didn't stick, re-do it in the current live session and save again — don't assume the first pass committed. (Adds usually persist; deletes are the ones most often lost to an intervening reload.) 
