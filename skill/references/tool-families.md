...
Pull and read public sbox.game packages. **Anonymous** — no login/session; only `Public` packages resolve (private → 404). A package ships its code in one of **three forms** (auto-detected → `result.recovery_method`), and `package_download` handles each: **`source_files`** — raw `.cs`/`.razor` shipped directly in the manifest (most **libraries**; you get the author's EXACT source + their REAL `.sbproj`, nothing to recover); **`cll_source`** — a `.cll` "code archive" (magic `GMCA` + LZ4 + JSON of raw C#/.razor) unpacked to exact source; **`decompiled_il`** — no source shipped (post-#5038 **games** ship precompiled `.bin/package.*.dll`), **decompiled** back to readable C# via ILSpy. Don't assume by package age — a 2026 library can ship source while a 2026 game ships a compiled DLL; the manifest decides.

- `package_search(query?, type?, sort?, limit?, skip?)`: **search OR browse** sbox.game. REST `/package/find/2`. `query` is **optional** — omit it and pass `sort` to browse a ranked feed (e.g. the most-played games right now). `type` (`game` for playable games) + `sort` are injected into the query DSL **server-side** (so `type:game` returns a full page of games, not a client-trimmed remnant); DSL tokens written straight into `query` (`type:game sort:popular +fps`) are still honored and take precedence. `sort` values: `popular`, `playersnow` (live players right now), `trending`, `quality`, `rankday`/`rankweek`/`rankmonth`, `newest`, `updated`, `bestrated`, `mostreviewed`, `hiddengem`, `favcount`, `random`. Each hit → `{ident (org.package), title, type, summary, org{ident,title}, users_now, favourited, votes_up, votes_down, updated, created, file_size_mb, thumb, tags[]}`; the result also carries `available_sorts` (the server's live list of offered orders) + `total_results`. Use this to **find popular games to download** then feed `ident` to `package_info`/`package_download`.
...
- `package_download(ident, output_dir?, recursive?, code_only?, unpack_code?, generate_project?, keep_archives?, overwrite?, concurrency?)`: engine-faithful pull to `{game}/.claude-sbox/downloads/<ident>/`. Per `recovery_method`: **`source_files`** → the raw `.cs`/`.razor` + the package's own real `.sbproj` are written as-is (no generation — best case). **`cll_source`** → `.cll` exploded to original source (`new Sandbox.CodeArchive(bytes)`, in-process) + a buildable `.sbproj` from the archive's `Compiler.Configuration`. **`decompiled_il`** → `_decompiled/<asm>.decompiled.cs` (in-process via ILSpy) + a best-effort `.sbproj`: `Type` from the package metadata, `RootNamespace`/`AssemblyReferences` recovered from PE metadata, `PackageReferences` (the implicit `base` addon is **omitted** — it's the always-mounted engine base, not a fetchable ident; listing it aborts a local load with `invalid ident`). The decompile is **high-fidelity + auto-cleaned to compilable C#** (see `decompiler_install`): resolved against the editor's runtime + engine assemblies (so `async`/`await` reconstruct and there are no `op_Implicit`/`<>k__BackingField`/static-extension-call/`(ref)`-cast fallbacks), then an s&box source-cleaner pass strips the IL-baked codegen the live generator re-emits (`[SourceLocation]`, generated `[Sync]`/`[ConVar]` members, generator-managed subscriber interfaces, redundant `[DefaultValue]`, explicit-interface accessors). Validated **24,086→6** compile errors on a 200k-line game (`thieves.terrortown`); the residual handful are genuine ILSpy nullable-inference glitches, not mechanical — so recovered games are editable in the editor, not just readable. **Post-decompile the recovered project is also made PLAYABLE, generically (no per-game logic):** (1) every Razor panel's `[StyleSheet]` is relinked to its on-disk `.scss` — decompilers can't reproduce the source-gen'd `[ClassFileLocation]` and drop the `[StyleSheet]` path to `null`, so OTHERWISE all UI renders unstyled/invisible (menus/buttons/HUD); `stylesheet_repairs`, toggle with `repair_decompiled`. (2) The project's boot/runtime **Metadata** is recovered from the package API (`Version.Meta` in the `/package/get/2` response = the original `.sbproj` Metadata block) and merged into the regenerated `.sbproj`: `SystemScene`/`StartupScene`/`MapStartupScene`/`GameNetworkType`/`MaxPlayers`/`Physics`/`TickRate`/`GameSettings`, none recoverable from IL. **Without `SystemScene` a recovered networked game loads its map but never loads the gamemode/system scene → no player ever spawns** (black/loading screen); recovering it makes the game boot. `recovered_project_metadata_keys`; entry points are surfaced as `startup_scene`/`system_scene` — to launch: open `startup_scene`, Play, host from its menu (the engine auto-loads `system_scene`'s gamemode). **Always pull to a FRESH/empty `output_dir`** — merging into a prior different-method download (e.g. an older `cll_source` tree) leaves stale `.cs`/`.razor` that double-define the decompiled `Code/` and break the build. Compile flags (`DefineConstants`/`NoWarn`/`Nullables`/…) are **intentionally omitted** — a real `.sbproj` doesn't store them (the editor applies `SANDBOX`/`ADDON`/`NoWarn` defaults at compile time) and they aren't recoverable from IL; `sbproj_needs_review` says so. Nothing is fabricated/guessed. Result fields: `recovery_method`, `source_files_shipped`/`sbproj_shipped` (source case), `source_files_extracted` (cll case), `assemblies_decompiled`/`decompiled_lines`/`recovered_root_namespace` (decompile case), `stylesheet_repairs`/`recovered_project_metadata_keys`/`startup_scene`/`system_scene` (decompiled-game playability). `recursive` (default on) pulls deps into `_packages/`; bundled archives → `_bundled/`; `code_only` = fast code-only grab (raw `.cs`/`.razor`/`.sbproj`/`.scss`, `.cll`, AND `.bin/*.dll`). `sanitize` (default **OFF**) runs the adversarial-naming neutralizer over the recovered source — see `source_sanitize`.
- `decompiler_install(force?, timeout_seconds?)`: one-time deploy of the decompiler driver (ICSharpCode.Decompiler / ILSpy) that the `.bin/*.dll` path needs. s&box's compiler ignores NuGet, so the decompiler can't be a compile-time dep — it's a prebuilt DLL fetched on demand (exactly like the codesearch Chromium driver) into `{game}/.claude-sbox/decompiler-driver/runtime/`, loaded in-process via `Assembly.LoadFrom`. Call when `package_download` reports `decompiler_unavailable`. Source + build script live in `claude-sbox-setup` (`decompiler-driver/`: `Entry.cs` wires the reference resolver + cleanup, `SboxSourceCleaner.cs` is the s&box-specific cleaner — 11 transforms), never in the published addon. Targets **net10** (the editor's runtime); a rebuild (`Build-Decompiler-Driver.sh`/`.bat`) needs a .NET 10 SDK, and `force:true` then an **editor restart** to swap an already-loaded driver. NOTE: these are **managed C#** so ILSpy is correct; the `ghidra-re` skill is native-only (fallback only if a package ships a native blob, which sbox.game packages don't).
- `source_sanitize(path, write?, strict?, include_strings?, include_comments?, extra_terms?, exclude_terms?)`: neutralize **adversarial alarming-vocabulary naming** in recovered source so you can work on it without being tripped. Some hostile packages name identifiers/strings with real-world WMD/CBRN/terror terms — sometimes disguised with **homoglyphs** (Cyrillic `пuke`, math-script `𝓃𝓊𝓀𝑒`) or **leetspeak** (`ch3mical`) — which survive decompilation (managed C# keeps author names). Canonicalizes (homoglyph-fold + leet-decode) → matches a curated denylist → **replaces with benign words** (`nuke`→`watermelon`) across identifiers, strings, interpolated strings, and comments, writing `claude-sbox-sanitize-map.json` so originals stay traceable. Denylist + Unicode fold map live in `claude-sbox-setup/sanitizer-data/` (loaded at runtime, tunable, NOT in the published addon). Default denylist = genuinely-dangerous real-world terms and **excludes** ordinary game-combat words (weapon/grenade/kill/sniper) — validated 0/25,318 collateral on a real game decompile; `strict:true` adds bomb/explosive/detonate/terrorist/etc. `write:false` (default) = dry-run report. Also the `sanitize` flag on `package_download` (default off). This is the defense for the "named to trip the agent" attack — distinct from `decompiled_il` *code* obfuscation below.

**Obfuscated `decompiled_il` output** (rare): a few packages run a *managed* C# obfuscator — encoded string literals (`Decode(new byte[]{…}, key)`), constant **opaque predicates** (`if (-88 > -90) { … }` wrapping the real body), mangled identifiers. s&box's was `f4industries.scfu` (now delisted). ILSpy still recovers the **full structure** — the obfuscation doesn't stop decompilation, it just hurts readability. Deobfuscate by **reading** (these transforms have clean inverses: decode the string blobs, fold the always-true predicates and keep the live branch) or run **`de4dot`** if it's a recognised obfuscator. **`ghidra-re` does NOT apply** — it's for *native* binaries; obfuscated managed code is still managed IL.
